...
def preprocess(rawdata, donormalize=0, istrain=1, traininfo=None, doengr=0):
    ...
    numels_norm = len(normfeats)
    if (donormalize == 1) and (numels_norm > 0):
        ...
        if istrain:
            ...
            mean_data = np.asarray(mean_data).reshape(1, numels_norm)
            std_data = np.asarray(std_data).reshape(1, numels_norm)
            # keep
